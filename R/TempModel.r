#' @export
TempModel = function(areas = 'subarea', annual.by.area=F, redo.data=F, redo=F){
if(redo){
	require(mgcv)
  ...
  TempData = TempModelData(redo=redo.data)
	...
	if(areas == 'lfa')		TempData$area = as.factor(TempData$LFA)
		...
		ou = list(Model=TempModel,Data=TempData)
		saveRDS(ou,file.path(project.datadirectory('bio.lobster'),'outputs','TempModelOutput.rds'))
}
  ou = readRDS(file.path(project.datadirectory('bio.lobster'),'outputs','TempModelOutput.rds'))
	return(ou)
}
